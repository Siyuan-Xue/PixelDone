...
Before release or cloud-infrastructure work, also read `RELEASE_SPEC.md`.
...
- `RELEASE_SPEC.md`: PixelDone-only GitHub/Gitee channels, RC naming, Supabase exceptions, SQL checkpoint, and release verification.
...
- The three standalone rule documents are exact mirrors of the PixelPark parent repository and must not be edited independently. Shared rule changes start in the parent and are copied here byte-for-byte. PixelDone-only durable release requirements belong in `RELEASE_SPEC.md`.
...
- Preserve the parent three standalone rule documents as the unique shared source of truth and verify the local mirrors before release.
...
Before release, verify the three local spec mirrors match the parent byte-for-byte and read `RELEASE_SPEC.md`.
